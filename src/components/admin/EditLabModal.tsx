...
export default function EditLabModal({
  ...
  useEffect(() => {
    ...
    setOpeningTime(lab.openingTime || "");
    setClosingTime(lab.closingTime || "");
  ...
  const handleSubmit = async (
    ...
    const isUnchanged =
      name.trim() === (lab.name || "").trim() &&
      description.trim() === (lab.description || "").trim() &&
      address.trim() === (lab.address || "").trim() &&
      phone.trim() === (lab.phone || "").trim() &&
      email.trim() === (lab.email || "").trim() &&
      openingTime === (lab.openingTime || "") &&
      closingTime === (lab.closingTime || "") &&
      JSON.stringify(images) === JSON.stringify(lab.images) &&
      JSON.stringify(services) === JSON.stringify(lab.services);

    if (isUnchanged) {
      setSubmitError("No changes were made to the lab");
      return;
    }
    ...
    try {
      ...
      const result = await fetchWithAuth<IUpdateLabRes>(
        `/api/labs/${lab._id}`,
        {
          method: "PATCH",
          body: {
      ...
      const updatedLab = result.data?.data?.lab;
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-main/10 bg-background px-6 py-5 dark:border-gray-700 dark:bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-foreground/60 dark:text-white/40">
                Update laboratory information and
                services.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="cursor-pointer rounded-lg p-2 text-foreground/50 transition hover:bg-main/10 hover:text-main disabled:cursor-not-allowed dark:text-white/50 dark:hover:bg-main/15"
          >
            <X size={20} />
          </button>
        </div>
        ...
        >
                  ...
                  {images.map((image, index) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute right-2 top-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg bg-red-500/90 text-white opacity-0 shadow-lg transition group-hover:opacity-100 hover:bg-red-600"
                      >
                        <Trash2 size={15} />
                      </button>
          ...
          <div className="rounded-xl border border-main/10 p-4 dark:border-gray-700">
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs text-foreground/50 dark:text-white/35">
                Update the basic details of this
                laboratory.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Hematology Laboratory"
          ...
          <div className="rounded-xl border border-main/10 p-4 dark:border-gray-700">
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs text-foreground/50 dark:text-white/35">
                Update the tests and services
                provided by this lab.
              </p>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex h-11 cursor-pointer items-center gap-1.5 rounded-xl bg-main px-4 text-sm font-medium text-white transition hover:bg-main/90"
              >
                <Plus size={17} />
                Add
              </button>
                ...
                {services.map((service, index) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="cursor-pointer text-foreground/40 transition hover:text-red-500 dark:text-white/40"
                    >
                      <X size={14} />
                    </button>
                  </div>
          ...
          <div className="flex justify-end gap-3 border-t border-main/10 pt-5 dark:border-gray-700">
            <button
              ...
              onClick={onClose}
              disabled={isSubmitting}
              className="cursor-pointer rounded-xl border border-main/10 px-5 py-2.5 text-sm font-medium text-foreground transition hover:bg-main/5 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700 dark:text-white dark:hover:bg-white/5"
            >
              Cancel
            </button>
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="flex min-w-[140px] cursor-pointer items-center justify-center gap-2 rounded-xl bg-main px-5 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-main/90 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isSubmitting && (
